...
def test_reader_can_create_review(api_client, book):
    ...
    data = {"book": book.id, "text": "Отличная книга!", "rating": 5}
...
def test_reader_cannot_see_unapproved_reviews(api_client, book):
    ...
    other = User.objects.create_user(
        email="another@lib.com", password="123", role="reader"
    )
    api_client.force_authenticate(user=user)

    Review.objects.create(
        book=book, user=other, text="Скрытый отзыв", rating=3, is_approved=False
    )
...
def test_admin_sees_all_reviews(api_client, book):
    ...
    Review.objects.create(
        book=book, user=user, text="Показать админу", rating=4, is_approved=False
    )
...
def test_owner_can_update_review(api_client, book):
    ...
    review = Review.objects.create(
        book=book, user=user, text="Старый текст", rating=3, is_approved=True
    )
...
def test_librarian_can_approve_review(api_client, book):
    ...
    librarian = User.objects.create_user(
        email="lib@lib.com", password="123", role="librarian"
    )
    user = User.objects.create_user(
        email="reader@lib.com", password="123", role="reader"
    )
    ...
    review = Review.objects.create(
        book=book, user=user, text="Нужна проверка", rating=4
    )
...
def test_others_cannot_approve_review(api_client, book):
    ...
    review = Review.objects.create(
        book=book, user=other_user, text="Кто ты такой?", rating=1
    )
